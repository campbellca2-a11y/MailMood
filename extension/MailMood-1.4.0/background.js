...
function lex(raw) {
  return raw.map((t) => {
    const [term, explicit] = typeof t === "string" ? [t, void 0] : t;
...
var LEX = {
  urgent: lex([
    ...
    ["don't delay", 1.4],
    ["can't wait", 1.3],
    ["running out of time", 1.5],
    ["drop everything", 1.6],
    ["top priority", 1.5],
    ["this cannot wait", 1.5],
    ["needs immediate attention", 1.5],
    ["respond immediately", 1.4],
    "time is running out",
    "act now",
    "action required",
    "response required",
  ...
  apologetic: lex([
    ...
    "hesitant",
    "uneasy",
    ["i take responsibility", 1.5],
    ["i should have", 1.3],
    ["i was wrong", 1.5],
    ["i feel bad", 1.4],
    ["i didn't mean", 1.3],
    ["please understand", 1.3],
    ["please don't be upset", 1.5],
    ["i hope this is okay", 1.3],
    ["i'm not sure if", 1.1],
    ["hope i haven't", 1.3],
    ["i owe you an apology", 1.5],
    ["this is my fault", 1.5],
    ["i take the blame", 1.5],
    ["i feel terrible", 1.4],
    ["i'm afraid that", 1.2],
    ["hope you can forgive", 1.4],
    ["didn't intend to", 1.2],
    ["i messed up", 1.4],
    ["i dropped the ball", 1.4]
  ]),
  warm: lex([
    "thanks",
    "thank you",
    ...
    ["love it", 1.3],
    ["so proud", 1.4],
    ["couldn't have done it without", 1.5],
    ["above and beyond", 1.4],
    ["hats off", 1.3],
    ["you rock", 1.4],
    ["made my day", 1.4],
    ["really impressed", 1.3],
    ["keep up the great", 1.3],
    ["so grateful", 1.4],
    ["truly appreciate", 1.4],
    ["can't thank you enough", 1.5],
    ["what a great", 1.3]
  ]),
  calm: lex([
    "please",
  ...
  sad: lex([
    ...
    ["passed away", 1.6],
    ["hard to accept", 1.3],
    ["difficult to process", 1.3],
    ["with heavy heart", 1.5],
    ["regret to inform", 1.6],
    ["it pains me", 1.4],
    ["i wish things were different", 1.4],
    ["i'm devastated", 1.5],
    ["breaks my heart", 1.5],
    ["thinking of you", 1.3],
    ["my heart goes out", 1.5],
    ["during this difficult", 1.4],
    ["in our thoughts", 1.3],
    "afraid",
    "fear",
    "dread"
  ...
  automated: lex([
    ...
    "opt out",
    "mailing list",
    ["this email was sent to", 1.5],
    ["you are receiving this", 1.5],
...
function getModifier(lower, idx) {
...
function scoreCategory(text, entries) {
...
function splitSentences(text) {
  return text.split(/(?<=[.!?])\s+|(?:\r?\n){1,}/).map((s) => s.trim()).filter((s) => s.length > 2);
}
function scoreSentences(sentences) {
  return sentences.map((s, i) => {
    const position = i === 0 ? "opening" : i === sentences.length - 1 ? "closing" : "body";
    return {
      text: s,
      scores: {
...
function aggregateScores(sentenceScores) {
  const totals = {
    urgent: 0,
    apologetic: 0,
    warm: 0,
...
function extractSignals(text, sentenceCount) {
...
function detectEmailStructure(text) {
  const lower = text.toLowerCase();
...
function analyzeTone(rawText) {
  ...
  const emotionalPeak = Math.max(
    catScores.urgent,
    catScores.apologetic,
    catScores.warm,
    catScores.sad
  );
  const strongEmotion = emotionalPeak > 1.2;
  const structureBonus = strongEmotion ? 0 : (structure.hasGreeting ? 0.25 : 0) + (structure.hasSignOff ? 0.25 : 0) + (sig.avgSentenceLen > 60 ? 0.2 : 0);
  const scoreCard = [
    {
      tone: "urgent_tense",
      value: catScores.urgent * 1.25 + sig.exclamationDensity * 1.6 + sig.capsRatio * 1.8
    },
    {
      tone: "apologetic_anxious",
      value: catScores.apologetic * 1.2 + sig.ellipsisDensity * 0.5
    },
    {
      tone: "warm_positive",
      value: catScores.warm * 1.15 + (sig.exclamationDensity > 0 && catScores.warm > 0 ? 0.4 : 0)
    },
    {
      tone: "calm_professional",
      value: catScores.calm * 1 + structureBonus + 0.35
      // baseline — human emails with no strong emotion default here
    },
    {
      tone: "sad_concerned",
      value: catScores.sad * 1.2 + sig.ellipsisDensity * 0.3
    },
    {
      tone: "neutral_automated",
      // Only wins when actual automation signals are present — no free baseline
      value: catScores.automated * 1.4 + (catScores.automated >= 2 ? 0.5 : 0)
      // bonus when multiple automation markers
    }
  ];
  ...
  const sentenceBonus = clamp(sentences.length * 0.02);
  const confidence = clamp(
    0.45 + gap * 0.09 + Math.min(0.22, top.value * 0.04) + sentenceBonus
  );
  ...
  const emotions = [
    ...
    },
    {
      ...
      intensity: clamp(
        catScores.calm * 0.1 + (structure.hasGreeting ? 0.1 : 0) + (structure.hasSignOff ? 0.1 : 0)
      )
    },
    {
      name: "stress",
...
function buildExplanation(tone, scores, sig, structure) {
  const parts = [];
  switch (tone) {
    case "urgent_tense":
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  try {
    if (msg.type === "PROCESS_TEXT") {
      ...
      if (msg.task === "rewrite") {
        const out2 = polishText(text);
        return sendResponse({ success: true, data: out2 });
      }
